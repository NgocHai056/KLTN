...
export class SeatService {
    ...
    async createManySeat(
        roomId: string, movieId: string, time: string, showtime: string, seatArray
    ) {
        ...
        return await this.showtimeService.update(
            showtimes[0].id,
            { $push: { seat_array: { $each: seatArray } } }, /** Sử dụng $push để thêm một phần tử mới*/
        );
    }


    async updateManySeat(
        roomId: string, movieId: string, time: string, showtime: string, seatArray: string[]
    ) {
    ...
    async checkEmptySeat(
        ...
        const existSeat = showtimes[0].seat_array.filter(seat => seatNumber.includes(seat.seat_number));
