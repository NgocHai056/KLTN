...
export class ComboService extends BaseService<Combo> {
    ...
    async creaetCombo(comboDto: ComboDto) {
        const products = await this.productService.findByIds(
            comboDto.combo_items.flatMap((product) => product.product_id),
        );
        ...
        products.forEach((product) => {
            ...
            description = description
                ? description + ", " + comboMap[product.id] + " " + product.name
                : comboMap[product.id] + " " + product.name;
        });
    ...
    async calculatePriceProduct(bookingComboDto: BookingComboDto[]) {
        const comboMap = new Map(
            bookingComboDto.map((combo) => [combo.combo_id, combo]),
        );
        ...
        return [
            ...comboDetails(
                await this.productService.findByIds(
                    bookingComboDto.flatMap((x) => x.combo_id),
                ),
            ),
        ];
    }
    ...
    async calculatePriceCombo(bookingComboDto: BookingComboDto[]) {
        const filteredCombos = bookingComboDto.filter(
            (combo) => combo.combo_type === ComboType.COMBO,
        );

        const filteredItems = bookingComboDto.filter(
            (item) => item.combo_type === ComboType.DRINK,
        );
        ...
        return [
            ...comboDetails(
                await this.findByIds(filteredCombos.flatMap((x) => x.combo_id)),
            ),
            ...comboDetails(
                await this.productService.findByIds(
                    filteredItems.flatMap((x) => x.combo_id),
                ),
            ),
        ];
    }
